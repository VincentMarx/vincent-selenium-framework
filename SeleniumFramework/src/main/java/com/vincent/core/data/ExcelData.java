...
public class ExcelData {
	...
	private static String SHEET_NAME = "TestCase";
	// key find start row, start column
	private static String HEADER_TEST_CASE = "TestCaseName";
	// 0 - not run, 1 - run one time, 2 - run 2 times
	private static String HEADER_RUNS = "Runs";
	private static String HEADER_STEPS = "Steps";
	private static String STEP_HEADER = "Step";
	...
	private List<TestCase> loadTestCase(File dataFile) {
		...
		if (isTestCaseHeaderFound) {
			for (int rowNum = startRow + 1; rowNum <= lastRowNum; rowNum++) {
				...
				int stepHeaderRowNum = startRow; // init stepHeaderRow = start
													// row
				...
				for (int stepParamCol = stepsCol + 1; stepParamCol <= stepHeaderRow.getLastCellNum(); stepParamCol++) {
	...
	public void setTestSet(List<TestCase> testSet) {
